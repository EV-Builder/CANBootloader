...
/*          #else //we need to remap if we want to have DUAL CANBUS
            gpio_set_mode(GPIO_BANK_CAN2_RX, GPIO_MODE_INPUT, GPIO_CNF_INPUT_PULL_UPDOWN, GPIO_CAN2_RX);
            gpio_set(GPIO_BANK_CAN2_RE_RX, GPIO_CAN2_RX);
...
void Can::HandleRx(int fifo)
{
   ...
   //while ( can_available_mailbox(canDev) )
   //{
